...
export class Tier1BM25 {
  ...
  async upsert(
    ...
    await database().execute(sql`
      INSERT INTO note_search (note_id, vault_id, title, body, tags, updated_at)
      VALUES (${noteId}, ${vaultId}, ${title}, ${body}, ${tags as unknown as string}::text[], NOW())
  ...
  async search(
    ...
    // running on plain pgvector/pgvector:pg16.
    const tokens = [
      ...new Set(
        query
          .toLowerCase()
